...
- 支持 Tavily、Firecrawl、TinyFish、Exa、Keenable 搜索和网页提取。
...
配置 ChatGPT2API 时，`web_search` 会先调用它。Tavily、Firecrawl、TinyFish、Exa、Keenable 构成默认补充和降级链，首个返回有效来源的 provider 即停止。请求包含域名或时间过滤条件时会跳过 Firecrawl，因为它的搜索接口无法严格执行这些过滤条件。Keenable 支持时间过滤和单个包含域名；排除域名或多个包含域名会明确报错，不会静默忽略。
...
可选值为 `chatgpt2api`、`tavily`、`firecrawl`、`tinyfish`、`exa`、`keenable`。指定的供应商必须已经配置；来源供应商还必须由 `HYBRID_SEARCH_SOURCE_PROVIDERS` 启用。指定来源供应商时，`extra_sources` 控制最大结果数量，默认使用 `HYBRID_SEARCH_FALLBACK_SOURCES`。
...
- 至少配置一个有效搜索来源：
  ...
  - 配置 `TAVILY_API_KEY`、`FIRECRAWL_API_KEY`、`TINYFISH_API_KEY`、`EXA_API_KEY`、`KEENABLE_API_KEY` 中任意一个。
...
| `KEENABLE_API_KEY` | — | Keenable API Key，通过 `X-API-Key` 发送。 |
| `KEENABLE_API_URL` | `https://api.keenable.ai` | [Keenable](https://docs.keenable.ai/) API 根地址。 |
...
| 环境变量 | 默认值 | 说明 |
| --- | --- | --- |
| `HYBRID_SEARCH_SOURCE_PROVIDERS` | `tavily,firecrawl,tinyfish,exa,keenable` | 逗号分隔的来源供应商顺序。可以调整顺序或省略供应商，名称无效时启动失败。ChatGPT2API 独立配置，默认仍最先调用。 |
...
对于匹配的 URL，`web_fetch` 会直接使用 GitHub REST API、Stack Exchange API v2.3、arXiv Export API 和 MediaWiki Action API。这些公共专用 API 不需要 Tavily、Firecrawl、TinyFish、Exa 或 Keenable 凭据。专用解析失败时，HybridSearch 会记录原因，并降级到已配置的通用抓取链。
